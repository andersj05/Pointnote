...
Version 1.2.0 adds optional `comparison` and screenshot detail/markup fields. A comparison has exactly two targets and zero-based `changeTarget` and `referenceTarget` indices that must be distinct. `dimension` is `overall`, `spacing`, `typography`, `color`, or `alignment`. The reference is intended to stay unchanged; the original comment remains authoritative. Explicit reattachment stores the previous relationship alongside its previous targets and requires the user to choose the two roles again.

An available screenshot may have `crops` (up to 12). Each crop identifies its zero-based `targetIndex`, or omits it for a selected area. Available crops carry their own PNG path, dimensions, captured viewport bounds, and `clipped` flag. Unavailable crops carry a reason. Native capture pixels are masked before any crop or resize; close-ups have at most 2,400 pixels per side, and aggregate close-up data is limited to 8 million data-URL characters. The viewport remains capped at 1,600 pixels wide. This capture mode never scrolls or claims to capture offscreen content.

The viewport and each available crop may retain `marks` and a separate `marked` PNG. Marks use normalized image coordinates in [0,1]: arrows have `from`/`to`, and callouts have `at` plus exact `text` (up to 240 characters). Callout numbers follow their order among callout marks for that image. Each image allows 40 marks. The studio renders from the original masked PNG and saves derived PNGs separately. ZIP export strips every inline data URL, writes original and marked PNGs, and includes callout text in both Markdown formats. Standalone Markdown cannot convey the image geometry and the handoff UI explains when to use ZIP.

`PATCH_SCREENSHOT` updates only a note's image edits in a storage transaction, with `capturePath` and `revision` checks to reject stale edits after another edit or reattachment. Review state and original comments remain intact. Reattachment history takes the latest stored screenshot so another tab's edits are preserved. Existing IndexedDB records require no migration because all new fields are optional. The total serialized annotation limit remains 16 million characters, including any derived images.
...
Backups have a separate `format: "pointnote-backup"`, `schemaVersion: "1.1.0"`, and full `annotations`/`sessions` arrays including inline original/marked PNG data. Restore also accepts legacy version 1.0.0. The new version prevents older installations from silently dropping the comparison and screenshot fields. This format is for local restoration; handoff ZIP JSON strips inline image data and references PNG files. Backup limits are 64 MB, 2,000 notes, 500 sessions, 12 targets per note, and 100 reattachments per note. Unknown/future backup versions, invalid shapes, duplicate IDs within a file, and unresolved session references are rejected before any write. Existing local IDs win during restore.

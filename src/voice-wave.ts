...
export function voiceWave() {
  ...
  return `<span class="voice-wave" aria-hidden="true">${heights.map((height, index) => `<span style="--wave-height:${height}px;--wave-delay:${-(index % 7) * 0.17}s;--wave-duration:${0.85 + (index % 5) * 0.16}s"></span>`).join('')}</span>`;
}
